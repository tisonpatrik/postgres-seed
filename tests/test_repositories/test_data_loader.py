...
async def test_fetch_data_as_dataframe_async():
    ...
    with patch.object(DataLoader, '_create_connection_pool', new_callable=AsyncMock) as mock_pool, \
         patch.object(DataLoader, '_execute_sql', new_callable=AsyncMock, return_value=mock_rows), \
         patch.object(DataLoader, '_convert_to_dataframe', return_value=pd.DataFrame(mock_rows)):
