...
def convert_datetime_to_unixtime(df: pd.DataFrame, date_time_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"Successfully converted column '{date_time_name}' to UNIX timestamp.")
        return df
        
    except Exception as e:
        logger.error(f"Error converting column '{date_time_name}' to UNIX timestamp: {e}")
        raise

def aggregate_to_day_based_prices(df: pd.DataFrame, index_column: str, price_column: str) -> pd.DataFrame:
    """
